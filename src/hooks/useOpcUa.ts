...
export function useCreateOpcUaSubscription() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ connectionId, ...data }: { connectionId: string; nodeId: string; mqttTopic: string; samplingIntervalMs?: number; brokerId?: string; destinationKind?: 'mqtt' | 'kafka'; connectorId?: string; kafkaHeaders?: Record<string, string>; publishRaw?: boolean }) =>
