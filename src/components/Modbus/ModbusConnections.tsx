...
export function ModbusConnections() {
  ...
  if (error) return <div className="bg-red-50 border border-red-100 rounded-xl px-4 py-3 text-[13px] text-red-500">Failed to load Modbus connections: {error.message}</div>;
          ...
          {connections.map((conn) => (
            <div key={conn.id} className="px-5 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
              ...
              <div className="flex items-center gap-1 ml-4 sm:ml-0 shrink-0">
                <button onClick={() => setConfiguringConn(conn)} className="px-2.5 py-1.5 text-[11px] font-medium text-gray-500 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors flex items-center gap-1">
