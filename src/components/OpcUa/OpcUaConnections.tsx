...
export function OpcUaConnections() {
  ...
  useEffect(() => {
    ...
    if (existing) {
      setPrefillBanner(t('opcua.connections.alreadyExists', { endpoint: existing.endpointUrl }));
  ...
  if (error) return <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800/30 rounded-xl px-4 py-3 text-[13px] text-red-500">{t('opcua.connections.loadError', { message: error.message })}</div>;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-end">
        <button onClick={() => setShowForm(true)} className="flex items-center gap-1.5 px-3.5 py-2 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-[13px] font-medium rounded-xl hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors">
          <Plus className="w-3.5 h-3.5" /> {t('opcua.connections.newConnection')}
        </button>
      </div>
      ...
      {(!connections || connections.length === 0) ? (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-800 px-6 py-12 text-center">
          <p className="text-[14px] text-gray-400">{t('opcua.connections.empty')}</p>
          <p className="text-[12px] text-gray-300 mt-1">{t('opcua.connections.emptyHint')}</p>
        </div>
      ) : (
            ...
            <div key={conn.id} className="px-5 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-gray-300 mt-0.5">{t('opcua.connections.security')}: {conn.securityMode}{conn.username ? ` · ${t('opcua.connections.authenticated')}` : ''}</p>
                </div>
              </div>
              <div className="flex items-center gap-1 ml-4 sm:ml-0 shrink-0">
                {conn.status === 'connected' && (
                  <>
                    <button onClick={() => setBrowsingConn(conn)} className="px-2.5 py-1.5 text-[11px] font-medium text-gray-500 bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors">{t('opcua.connections.browse')}</button>
                    <button onClick={() => setMonitoringConn(conn)} className="px-2.5 py-1.5 text-[11px] font-medium text-gray-500 bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors flex items-center gap-1">
                      <Activity className="w-3 h-3" /> {t('opcua.connections.monitor')}
                    </button>
                ...
                <button onClick={() => setEditConn(conn)} title={t('opcua.connections.editConnection')} className="p-2 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                  <Pencil className="w-3.5 h-3.5" />
                </button>
                <button onClick={() => { if (confirm(t('opcua.connections.confirmDelete'))) deleteMutation.mutate(conn.id); }} disabled={deleteMutation.isPending} className="p-2 rounded-lg text-red-300 hover:text-red-500 hover:bg-red-50 transition-colors">
