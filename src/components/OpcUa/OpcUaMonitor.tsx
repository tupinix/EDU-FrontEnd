    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <input type="text" placeholder="NodeId for on-demand read (e.g. ns=2;i=2)" value={readNodeId}
          ...
          className="flex-1 px-3.5 py-2 text-[13px] font-mono bg-gray-50 border border-gray-100 rounded-xl outline-none placeholder:text-gray-300 focus:border-gray-200 focus:ring-2 focus:ring-gray-100 transition-all" />
        <button onClick={() => readMutation.mutate()} disabled={!readNodeId.trim() || readMutation.isPending}
          className="flex items-center gap-1.5 px-3.5 py-2 bg-gray-900 text-white text-[12px] font-medium rounded-xl hover:bg-gray-800 transition-colors disabled:opacity-30">
      ...
      {readMutation.error && <div className="text-[13px] text-red-500 bg-red-50 border border-red-100 rounded-xl px-4 py-3">{(readMutation.error as Error).message}</div>}
...
function MonitorRow({ v }: { v: NodeLiveValue }) {
  ...
  return (
    <tr className="border-t border-gray-50">
      <td className="px-5 py-2 font-mono text-gray-600 truncate max-w-[250px]" title={v.nodeId}>{v.nodeId}</td>
...
function ReadResult({ value }: { value: NodeLiveValue }) {
  return (
    <div className="bg-gray-50 rounded-xl px-4 py-3 flex flex-wrap gap-x-5 gap-y-1 text-[12px]">
      <span className="font-mono text-gray-700 font-medium">{value.nodeId}</span>
      <span><span className="text-gray-400">Value:</span> <strong className="text-gray-900">{fmtVal(value.value)}</strong></span>
