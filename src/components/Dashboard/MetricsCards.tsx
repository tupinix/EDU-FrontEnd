...
function Metric({ label, value, suffix }: { label: string; value: string | number; suffix?: string }) {
  return (
    <div className="px-5 py-4 sm:px-6 sm:py-5">
      ...
      <p className="text-xl sm:text-2xl font-semibold text-gray-900 tracking-tight mt-1">
        {typeof value === 'number' ? value.toLocaleString('pt-BR') : value}
        {suffix && <span className="text-xs sm:text-sm font-normal text-gray-300 ml-1">{suffix}</span>}
      </p>
    </div>
  );
}
...
export function MetricsCards({ messagesPerDay, messagesPerMinute, totalTopics, errorRate }: MetricsCardsProps) {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 bg-white rounded-2xl border border-gray-200/60 divide-y sm:divide-y-0 sm:divide-x divide-gray-100 overflow-hidden">
