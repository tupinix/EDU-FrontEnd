        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl px-4 py-3">
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-[10px] font-mono text-blue-600 bg-blue-50 dark:bg-blue-900/30 dark:text-blue-400 px-1.5 py-0.5 rounded">{tag.data_type}</span>
            </div>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-[15px] font-semibold text-gray-900 dark:text-gray-100">Tag Browser — {connection.name}</h3>
          <p className="text-[11px] text-gray-400 font-mono truncate">{connection.host} &middot; Slot {connection.slot}</p>
        </div>
        <button onClick={handleRefresh} disabled={isFetching}
          className={cn('flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[11px] font-medium transition-colors bg-gray-50 dark:bg-gray-800/50 text-gray-400 hover:text-gray-500', isFetching && 'opacity-50')}>
      ...
      {showSubs && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-800 overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-100 dark:border-gray-800"><p className="text-[12px] font-semibold text-gray-900 dark:text-gray-100">Subscribed tags</p></div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-800 overflow-hidden">
          <div className="hidden sm:grid sm:grid-cols-[1fr_auto_auto_auto] gap-2 px-5 py-2.5 bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800 text-[10px] font-medium text-gray-400 uppercase tracking-wider">
          ...
          <div className="divide-y divide-gray-50 dark:divide-gray-800/50 max-h-[60vh] overflow-y-auto">
            {filteredTags.map((tag, i) => {
              ...
              return (
                <div key={i} className={cn('grid grid-cols-[1fr_auto_auto_auto] gap-2 items-center px-5 py-2 hover:bg-gray-50/50 dark:hover:bg-gray-800 transition-colors', subscribed && 'bg-blue-50/30 dark:bg-blue-900/10')}>
                  <div className="flex items-center gap-2 min-w-0">
                    <span className={cn('w-1.5 h-1.5 rounded-full shrink-0', subscribed ? 'bg-blue-400' : 'bg-gray-300')} />
                    <span className="text-[12px] font-medium text-gray-900 dark:text-gray-100 font-mono truncate">{tag.tag_name}</span>
                    {subscribed && <Rss className="w-2.5 h-2.5 text-blue-400 shrink-0" />}
                  </div>
                  <span className="text-[10px] font-mono text-blue-600 bg-blue-50 dark:bg-blue-900/30 dark:text-blue-400 px-1.5 py-0.5 rounded shrink-0">{tag.data_type}</span>
