...
import { useCreateEthipConnection, useUpdateEthipConnection } from '../../hooks/useEthernetIp';
...
export function EthernetIpForm({ onClose, initialValues, editId }: EthernetIpFormProps) {
  const createMutation = useCreateEthipConnection();
  const updateMutation = useUpdateEthipConnection();
  const isEdit = !!editId;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-800 shadow-xl w-full max-w-lg">
        <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between">
          <div>
            <h3 className="text-[15px] font-semibold text-gray-900 dark:text-gray-100">{isEdit ? 'Edit EtherNet/IP Connection' : 'New EtherNet/IP Connection'}</h3>
