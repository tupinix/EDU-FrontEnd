...
export function KafkaConnections() {
  ...
  const testMut = useMutation({
    ...
    onSuccess: (r, id) => setTestResult((p) => ({ ...p, [id]: r.ok ? `OK · ${r.latencyMs}ms` : (r.error ?? 'falhou') })),
  ...
  if (error) return <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800/30 rounded-xl px-4 py-3 text-[13px] text-red-500">Falha ao carregar conectores Kafka: {error.message}</div>;
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-end">
        <button onClick={openCreate} className="flex items-center gap-1.5 px-3.5 py-2 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-[13px] font-medium rounded-xl hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors">
          <Plus className="w-3.5 h-3.5" /> Novo conector
        </button>
      </div>
      ...
      {showForm && (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-800 p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-[14px] font-semibold text-gray-900 dark:text-gray-100">{editId ? 'Editar conector' : 'Novo conector Kafka'}</h3>
            <button onClick={closeForm} className="p-1 text-gray-400 hover:text-gray-600"><X className="w-4 h-4" /></button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Field label="Nome"><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="redpanda-prod" className={inputCls} /></Field>
            <Field label="Bootstrap servers"><input value={form.bootstrapServers} onChange={(e) => setForm({ ...form, bootstrapServers: e.target.value })} placeholder="localhost:9092" className={inputCls} /></Field>
            ...
            {isSasl && (
              <>
                ...
                <Field label="SASL username"><input value={form.saslUsername ?? ''} onChange={(e) => setForm({ ...form, saslUsername: e.target.value })} className={inputCls} /></Field>
                <Field label="SASL password"><input type="password" value={form.saslPassword ?? ''} onChange={(e) => setForm({ ...form, saslPassword: e.target.value })} placeholder={editId ? '•••• (manter)' : ''} className={inputCls} /></Field>
              </>
            )}
            {isConsumer && (
              <Field label="Consumer group"><input value={form.consumerGroupId ?? ''} onChange={(e) => setForm({ ...form, consumerGroupId: e.target.value })} placeholder="edu-consumer" className={inputCls} /></Field>
            )}
            <Field label="Produce topics (csv)"><input value={(form.produceTopics ?? []).join(',')} onChange={(e) => setForm({ ...form, produceTopics: csv(e.target.value) })} className={inputCls} /></Field>
            <Field label="Consume topics (csv)"><input value={(form.consumeTopics ?? []).join(',')} onChange={(e) => setForm({ ...form, consumeTopics: csv(e.target.value) })} className={inputCls} /></Field>
          </div>
          {(createMut.error || updateMut.error) && <p className="text-[12px] text-red-500">{(createMut.error || updateMut.error)?.message}</p>}
          <div className="flex justify-end gap-2">
            <button onClick={closeForm} className="px-3.5 py-2 text-[13px] text-gray-500 hover:text-gray-700">Cancelar</button>
            <button onClick={submit} disabled={saving || !form.name || !form.bootstrapServers} className="flex items-center gap-1.5 px-3.5 py-2 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-[13px] font-medium rounded-xl disabled:opacity-50">
              {saving && <Loader2 className="w-3.5 h-3.5 animate-spin" />} {editId ? 'Salvar' : 'Criar'}
            </button>
          </div>
        </div>
      )}
      ...
      {(!connectors || connectors.length === 0) ? (
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-800 px-6 py-12 text-center">
          <p className="text-[14px] text-gray-400">Nenhum conector Kafka</p>
          <p className="text-[12px] text-gray-300 mt-1">Adicione um cluster (Kafka ou Redpanda) para publicar o UNS</p>
        </div>
      ) : (
          ...
          {connectors.map((c) => (
            <div key={c.id} className="px-5 sm:px-6 py-4 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {c.isActive && <span className="text-[10px] px-1.5 py-0.5 rounded-md font-medium bg-amber-50 text-amber-600 dark:bg-amber-900/20 dark:text-amber-400 flex items-center gap-0.5"><Star className="w-2.5 h-2.5 fill-current" /> ativo</span>}
                  </div>
              ...
              <div className="flex items-center gap-1 ml-4 sm:ml-0 shrink-0">
                <button onClick={() => testMut.mutate(c.id)} disabled={testMut.isPending} title="Testar conexão" className="px-2.5 py-1.5 text-[11px] font-medium text-gray-500 bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors flex items-center gap-1">
                  <Gauge className="w-3 h-3" /> Test
                </button>
                {!c.isActive && <button onClick={() => activateMut.mutate(c.id)} title="Tornar ativo" className="p-2 rounded-lg text-amber-400 hover:text-amber-600 hover:bg-amber-50 transition-colors"><Star className="w-3.5 h-3.5" /></button>}
                {c.status === 'connected'
                  ? <button onClick={() => disconnectMut.mutate(c.id)} disabled={disconnectMut.isPending} title="Desconectar" className="p-2 rounded-lg text-amber-400 hover:text-amber-600 hover:bg-amber-50 transition-colors"><Unplug className="w-3.5 h-3.5" /></button>
                  : <button onClick={() => connectMut.mutate(c.id)} disabled={connectMut.isPending} title="Conectar" className="p-2 rounded-lg text-emerald-400 hover:text-emerald-600 hover:bg-emerald-50 transition-colors">{connectMut.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Plug className="w-3.5 h-3.5" />}</button>}
                <button onClick={() => openEdit(c)} title="Editar" className="p-2 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
                <button onClick={() => { if (confirm('Excluir este conector?')) deleteMut.mutate(c.id); }} disabled={deleteMut.isPending} title="Excluir" className="p-2 rounded-lg text-red-300 hover:text-red-500 hover:bg-red-50 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            </div>
          ))}
        </div>
