...
export function VirtualSensorsPage() {
  ...
  async function runTest() {
    try {
      ...
      setTestResult(`OK · com todas entradas = 1 → ${r.result}`);
    } catch (e) { setTestResult(e instanceof Error ? e.message : 'expressão inválida'); }
  }
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      <div>
        <h1 className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-gray-100 tracking-tight">Sensores Virtuais</h1>
        <p className="text-[12px] sm:text-[13px] text-gray-400 mt-0.5">Tags derivadas por fórmula sobre outras tags, publicadas no UNS como cidadãs de 1ª classe (historizam, alarmam, viram widget)</p>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-end">
          <button onClick={openCreate} className="flex items-center gap-1.5 px-3.5 py-2 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-[13px] font-medium rounded-xl hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors">
            <Plus className="w-3.5 h-3.5" /> Novo sensor
          </button>
        </div>
        ...
        {showForm && (
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-800 p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-[14px] font-semibold text-gray-900 dark:text-gray-100">{editId ? 'Editar sensor' : 'Novo sensor virtual'}</h3>
              <button onClick={closeForm} className="p-1 text-gray-400 hover:text-gray-600"><X className="w-4 h-4" /></button>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label="Nome"><input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="OEE Linha 1" className={inputCls} /></Field>
              <Field label="Tópico de saída (UNS)"><input value={form.outputTopic} onChange={(e) => setForm({ ...form, outputTopic: e.target.value })} placeholder="virtual/oee" className={cn(inputCls, 'font-mono')} /></Field>
              <Field label="Fórmula"><input value={form.expression} onChange={(e) => setForm({ ...form, expression: e.target.value })} placeholder="a * b / 100" className={cn(inputCls, 'font-mono')} /></Field>
              <Field label="Unidade"><input value={form.unit ?? ''} onChange={(e) => setForm({ ...form, unit: e.target.value })} placeholder="%" className={inputCls} /></Field>
            </div>

            <div>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[11px] font-medium text-gray-500 dark:text-gray-400">Entradas (variável → tag)</span>
                <button onClick={addInput} className="text-[12px] text-gray-500 hover:text-gray-900 dark:hover:text-gray-100 flex items-center gap-1"><Plus className="w-3 h-3" /> entrada</button>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label="Intervalo (ms)"><input type="number" min={1000} value={form.intervalMs} onChange={(e) => setForm({ ...form, intervalMs: Number(e.target.value) })} className={inputCls} /></Field>
              <label className="flex items-center gap-2 text-[13px] text-gray-600 dark:text-gray-300 self-end pb-2">
                <input type="checkbox" checked={form.enabled} onChange={(e) => setForm({ ...form, enabled: e.target.checked })} /> Ativo
              </label>
            </div>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={runTest} className="flex items-center gap-1.5 px-3.5 py-2 text-[13px] text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"><Play className="w-3.5 h-3.5" /> Testar fórmula</button>
        ...
        {isLoading ? (
          ...
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800/30 rounded-xl px-4 py-3 text-[13px] text-red-500">Falha ao carregar: {error.message}</div>
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200/60 dark:border-gray-800 px-6 py-12 text-center">
            <p className="text-[14px] text-gray-400">Nenhum sensor virtual</p>
            <p className="text-[12px] text-gray-300 mt-1">Crie uma tag derivada. Ex: <span className="font-mono">OEE = a * b / 100</span></p>
          </div>
        ) : (
                ...
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {!s.enabled && <span className="text-[10px] px-1.5 py-0.5 rounded-md font-medium bg-gray-100 text-gray-400 dark:bg-gray-800">pausado</span>}
                    </div>
                    <p className="text-[12px] text-gray-400 font-mono truncate mt-0.5">{s.expression} → {s.outputTopic}</p>
                    <p className="text-[11px] text-gray-300 mt-0.5">
                      {s.inputs.map((i) => `${i.var}=${i.topic}`).join(' · ') || 'sem entradas'} · a cada {s.intervalMs / 1000}s
                      {s.status?.lastError
                        ? <span className="ml-1 text-red-400">· {s.status.lastError}</span>
                        : s.status?.lastValue != null && <span className="ml-1 text-gray-400">· valor {s.status.lastValue}{s.unit ? ` ${s.unit}` : ''}</span>}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-1 ml-4 sm:ml-0 shrink-0">
                  <button onClick={() => openEdit(s)} title="Editar" className="p-2 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
                  <button onClick={() => { if (confirm('Excluir este sensor?')) deleteMut.mutate(s.id); }} disabled={deleteMut.isPending} title="Excluir" className="p-2 rounded-lg text-red-300 hover:text-red-500 hover:bg-red-50 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              </div>
            ))}
          </div>
